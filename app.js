...
class App {
  ...
  onclickBtnEvent() {
    ...
    this.registBtn.addEventListener('click', () => this.overWriteTxt())
  ...
  overWriteTxt() {
    console.log('OK押下')
    // textboxに上書きする
    const overWriteTxt = document.getElementsByClassName(CLASS_OVER_WRITE)[this.selectIndex].textContent;
    this.textBox.value = overWriteTxt;
    this.countTxtLength();

    // 選択行を着色する
    const overWriteTxtBlock = document.getElementsByClassName(`${CLASS_BLOCK_OVER_WRITE}`)[this.selectIndex];
    ...
    overWriteTxtBlock.closest('div').classList.add(CLASS_SELECT);
